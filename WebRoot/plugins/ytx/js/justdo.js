...
(function() {
	window.IM = window.IM || {
        ...
        _appid : '8a216da857ad33250157d195a9f32136', // 20150314000000110000000000000010应用I
...
//			RL_YTX.unbindBeforeUnLoad(s);
		...
		xl_DO_login: function(user_account) {debugger;
			console.log("DO_login");
		...
		_login: function(user_account, pwd) {
			...
			if (flag) {
				...
				var appToken = 'fb83c69663a119f28b3d314a8be0d8b7'; //使用是赋值为应用对应的appToken
//				var appToken = '59b96f26a230f1126bd18df3df8ab877';//开发token
...
//			if (!declared) {
//				IM.HTML_showAlert('alert-error', '公告不能为空');
//				return false;
//			}
			...
			//公告验证
//			var regx2 = /^[\\x00-\\x7F\a-zA-Z\u4e00-\u9fa5。；，：“”（）、？《》]+$/;
//			if (regx2.exec(declared) == null) {
//				IM.HTML_showAlert('alert-error',
//					'必须是大小写字母数字和ascii码值以及。；，：“”（）、？《》');
//				return false;
//			}
		...
		DO_push_createMsgDiv: function(obj) {
				...
				if (1 == msgType) {
                	...
                	*/
					...
					if (isFireMsg & inforSender != IM._user_account) {
						...
						var str = '<img src="' + url + '" style="cursor:pointer;max-width:' + imgWid + 'px; max-height:' + imgHei + 'px;" onclick="IM.DO_pop_phone(\'' + you_sender + '\', \'' + version + '\')"/>';
					}
					...
					/*RL_YTX.getFileSource(url, function(obj) {//功能待完善
						console.log(obj);
						url = obj.url;
					}, function(obj) {
						if (obj.code) {
					...
					if (isFireMsg) {
						str = '<div style="display:inline"><a fireMsg="yes" href="' + url + '" target="_blank">' + '<span>' + '<img style="width:32px; height:32px; margin-right:5px; margin-left:5px;" src="assets/img/attachment_icon.png" />' + '</span>' + '<span>' + fileName + '</span>' //+ '<span style="font-size: small;margin-left:15px;">'+size+'</span>'
							+ '</a>' + '<span style="font-size: small;margin-left:15px;">' + size + '</span></div>';
					} else {
						str = '<div style="display:inline"><a href="' + url + '" target="_blank">' + '<span>' + '<img style="width:32px; height:32px; margin-right:5px; margin-left:5px;" src="assets/img/attachment_icon.png" />' + '</span>' + '<span>' + fileName + '</span>' //+ '<span style="font-size: small;margin-left:15px;">'+size+'</span>'
							+ '</a>' + '<span style="font-size: small;margin-left:15px;">' + size + '</span></div>';
					}
			...
			if (53 == obj.mcmEvent) { // 3发送mcm消息
				...
				if (1 == msgType) {
					...
					var url = obj.msgFileUrl;
					if (isFireMsg) {
						str = '<audio fireMsg="yes" style="display:none" controls="controls" src="' + url + '">your browser does not surpport the audio element</audio>';
					} else {
					...
					if (isFireMsg) {
						str = '<img fireMsg="yes" onclick="IM.DO_pop_phone(\'' + you_sender + '\', \'' + version + '\')" videourl="' + urlReal + '" src="' + urlShow + '" style="max-width:' + imgWid + 'px;max-height:' + imgHei + 'px;display:none" />';
					} else {
						str = '<img onclick="IM.DO_pop_phone(\'' + you_sender + '\', \'' + version + '\')" videourl="' + urlReal + '" src="' + urlShow + '" style="max-width:' + imgWid + 'px;max-height:' + imgHei + 'px;" />';
					}
				} else if (4 == msgType) { // 4：图片消息
					...
					var fileName = obj.msgFileName;
					if (isFireMsg) {
						str = '<a fireMsg="yes" style="display:none" href="' + url + '" target="_blank">' + '<span>' + '<img style="width:32px; height:32px; margin-right:5px; margin-left:5px;" src="assets/img/attachment_icon.png" />' + '</span>' + '<span>' + fileName + '</span>' + '<span style="font-size: small;margin-left:15px;">' + size + '</span>' + '</a>';
					} else {
						str = '<a href="' + url + '" target="_blank">' + '<span>' + '<img style="width:32px; height:32px; margin-right:5px; margin-left:5px;" src="assets/img/attachment_icon.png" />' + '</span>' + '<span>' + fileName + '</span>' + '<span style="font-size: small;margin-left:15px;">' + size + '</span>' + '</a>';
					}
				}
...
//			var jkl = 0;
//			var autoUserState = setInterval(function(e){
//				if(jkl > 100){
//					clearInterval(autoUserState);
//				}else{
//					jkl++;
//				}
//				console.log(jkl);
...
//			
		...
		HTML_addContactToList: function(contactVal, contactName, content_type,
			...
			var str = '<li onclick="IM.DO_chooseContactList(\'' + content_type + '\', \'' + contactVal + '\',' + 'true)" id="im_contact_' + contactVal + '" im_isnotice="' + isNotice + '" contact_type="' + content_type + '" contact_you="' + contactVal + '" class="' + active + '"  style="display:' + dis + '">' + '<a href="javascript:void(0);">' + '<span contact_style_type="name">' + contactName + '</span>';
			if (IM._contact_type_g == content_type) {
				if (contactName != "系统通知") {
					str += '<span class="pull-right" onclick="IM.DO_groupMenu(\'' + contactVal + '\', \'' + owner + '\', ' + target + ');"><i class="icon-wrench"></i></span>';
				}

			}

			str += '<span contact_style_type="warn" class="badge badge-warning pull-right" style="margin-top:3px; display:none;">0</span>' + '<span contact_style_type="onlineState" class="badge pull-right" style="margin-top:6px; margin-right:10px; display:none;"></span>' + '</a>' + '</li>';
			$('#im_contact_list').prepend(str);
		...
		DO_checkNick: function(obj) {
			...
			if ('' == nick.trim()) {
				...
				//var regx = /^[\\x00-\\x7F\a-zA-Z\u4e00-\u9fa5_-]{0,64}$/;//昵称验证
				if (nick == null) {
					alert("昵称不能为空");
					return;
				} else {
		 ...
		 * @constructor
		 */
		DO_choose_contact_type: function(contact_type) {debugger;
			$('#im_contact_type').find('li').each(function() {
				$(this).removeClass('active');
